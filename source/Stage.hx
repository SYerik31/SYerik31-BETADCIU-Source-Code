...
class Stage extends MusicBeatState
{
	...
	public function new(daStage:String)
	{
		...
		switch (daStage)
		{
			...
			case 'hexwd':
				{
					camZoom = 0.9;
					curStage = 'hexwd';
					swagBacks['hexBack'] = new FlxSprite(-24, 24).loadGraphic(Paths.image('hex/weekend/detected/hexBack'));
					...
					swagBacks['hexFront'] = new FlxSprite(-24, 24).loadGraphic(Paths.image('hex/weekend/detected/hexFront'));
					...
					swagBacks['topOverlay'] = new FlxSprite(-24, 24).loadGraphic(Paths.image('hex/weekend/detected/topOverlay'));
					...
					sprite.frames = Paths.getSparrowAtlas('hex/weekend/detected/crowd');
			...
			case 'dokiclubroom-sayori' | 'dokiclubroom-natsuki' | 'dokiclubroom-yuri' | 'dokiclubroom-monika' | 'dokiclubroom2' | 'dokiclubroom3' | 'dokiclubroom-empty' | 'dokizoom':
			{
				camZoom = 0.75;
				...
				switch (daStage)
				{
					...
					case "dokiclubroom-empty":
					{
						swagBacks['empty'] = empty;
						toAdd.push(empty);
						empty.x = 1050;
						empty.y = 250;
					}
			...
			case 'dokifestival':
				{
					...
					lights_front.frames = Paths.getSparrowAtlas('doki/festival/lights_front', 'shared');
					...
					deskfront = new FlxSprite(posX, posY).loadGraphic(Paths.image('doki/festival/DesksFestival', 'shared'));
					deskfront.setGraphicSize(Std.int(deskfront.width * 1.6));
					deskfront.updateHitbox();
					deskfront.antialiasing = true;
					deskfront.scrollFactor.set(1.3, 0.9);

					closet = new FlxSprite(posX, posY).loadGraphic(Paths.image('doki/festival/FarBack', 'shared'));
					closet.setGraphicSize(Std.int(closet.width * 1.6));
					closet.updateHitbox();
					closet.antialiasing = true;
					closet.scrollFactor.set(0.9, 0.9);
					...
					clubroom = new FlxSprite(posX, posY).loadGraphic(Paths.image('doki/festival/MainBG', 'shared'));
					clubroom.setGraphicSize(Std.int(clubroom.width * 1.6));
					clubroom.updateHitbox();
					clubroom.antialiasing = true;
					clubroom.scrollFactor.set(1, 0.9);
					...
					lights_back.frames = Paths.getSparrowAtlas('doki/festival/lights_back', 'shared');
					...
					monika.frames = Paths.getSparrowAtlas('doki/bgdoki/monika', 'shared');
					monika.animation.addByPrefix('idle', "Moni BG", 24, false);
					monika.antialiasing = true;
					monika.scrollFactor.set(1, 0.9);
					monika.setGraphicSize(Std.int(monika.width * .7));
					...
					sayori.frames = Paths.getSparrowAtlas('doki/bgdoki/sayori', 'shared');
					sayori.animation.addByPrefix('idle', "Sayori BG", 24, false);
					sayori.antialiasing = true;
					sayori.scrollFactor.set(1, 0.9);
					sayori.setGraphicSize(Std.int(sayori.width * .7));
					...
					natsuki.frames = Paths.getSparrowAtlas('doki/bgdoki/natsuki', 'shared');
					natsuki.animation.addByPrefix('idle', "Natsu BG", 24, false);
					natsuki.antialiasing = true;
					natsuki.scrollFactor.set(1, 0.9);
					natsuki.setGraphicSize(Std.int(natsuki.width * .7));
					...
					protag.frames = Paths.getSparrowAtlas('doki/bgdoki/protag', 'shared');
					protag.animation.addByPrefix('idle', "Protag-kun BG", 24, false);
					protag.antialiasing = true;
					protag.scrollFactor.set(1, 0.9);
					protag.setGraphicSize(Std.int(protag.width * .7));
					...
					yuri.frames = Paths.getSparrowAtlas('doki/bgdoki/yuri', 'shared');
					yuri.animation.addByPrefix('idle', "Yuri BG", 24, false);
					yuri.antialiasing = true;
					yuri.scrollFactor.set(1, 0.9);
					yuri.setGraphicSize(Std.int(yuri.width * .7));
            ...
            case 'mii-channel':
            {
				...
				stageCurtains.frames = Paths.getSparrowAtlas('miibuttons');
			...
			case 'woods':
			{
				camZoom = 0.77;
				curStage = 'woods';
				var sky:FlxSprite = new FlxSprite(150, -20).loadGraphic(Paths.image('woods/sky'));
				...
				var woods:FlxSprite = new FlxSprite(-550, -230).loadGraphic(Paths.image('woods/woods'));
				woods.antialiasing = FlxG.save.data.antialiasing;
				woods.scrollFactor.set(1, 1);
				woods.active = false;
				swagBacks['woods'] = woods;
				toAdd.push(woods);
		    }
