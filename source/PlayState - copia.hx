...
class PlayState extends MusicBeatState
{
	...
	var bakaOverlayMad:FlxSprite;
	var SkepticSnake:FlxSprite;
	var ByeByeSnake:FlxSprite;
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
            ...
            case 'mii-channel':
					{
						...
						var bg:FlxSprite = new FlxSprite(-550, -230).loadGraphic(Paths.image('eteled/stageback'));
			...
			case 'dokiclubroom':
				...
				var empty = new FlxSprite(0, 0);
				empty.frames = Paths.getSparrowAtlas('doki/bgdoki/empty');
				empty.animation.addByPrefix('idle', "Empty BG", 24, false);
				empty.antialiasing = true;
				empty.scrollFactor.set(1, 0.9);
				empty.setGraphicSize(Std.int(empty.width * .7));
				empty.updateHitbox();
		...
		if (duoDad)
		{
			switch (SONG.song.toLowerCase())
			{
				...
				default:
					dad1 = new Character(30, 490, 'cerbera-ex');
			}				
		}
		...
		if (triple)
		{
			dad1 = new Character(40, 450, 'cerbera-ex');
		...
		if (SONG.song.toLowerCase() == 'killer-cough')
		{
		...
		if (SONG.song.toLowerCase() == 'baka') {
			bakaOverlay = new FlxSprite(300, 150);
			bakaOverlay.scale.set(1.8, 1.8);
			bakaOverlay.frames = Paths.getSparrowAtlas('doki/clubroom/BakaBGDoodles', 'shared');
			bakaOverlay.animation.addByPrefix('normal', 'Normal Overlay', 24, true);
			bakaOverlay.animation.addByPrefix('party rock is', 'Rock Overlay', 24, true);
		...
		if (SONG.song.toLowerCase() == 'baka') {
			bakaOverlayMad = new FlxSprite(300, 150);
			bakaOverlayMad.scale.set(1.8, 1.8);
			bakaOverlayMad.frames = Paths.getSparrowAtlas('doki/clubroom/BakaBGDoodles', 'shared');
			bakaOverlayMad.animation.addByPrefix('normal', 'Normal Overlay', 24, true);
			bakaOverlayMad.animation.addByPrefix('party rock is', 'Rock Overlay', 24, true);
			bakaOverlayMad.cameras = [camHUD];
			bakaOverlayMad.antialiasing = true;
			bakaOverlayMad.animation.play('party rock is');
			add(bakaOverlayMad);
			if (!executeModchart) {
				bakaOverlayMad.alpha = 0;
			}
		} 

		if (SONG.song.toLowerCase() == 'intertwined-ex') {
			SkepticSnake = new FlxSprite(300, 150);
			SkepticSnake.scale.set(1.8, 1.8);
			SkepticSnake.frames = Paths.getSparrowAtlas('SkepticSnake', 'shared');
			SkepticSnake.animation.addByPrefix('normal', 'Normal Overlay', 24, true);
			SkepticSnake.cameras = [camHUD];
			SkepticSnake.antialiasing = true;
			SkepticSnake.animation.play('normal');
			add(SkepticSnake);
			if (!executeModchart) {
				SkepticSnake.alpha = 0;
			}
		} 
		...
		if (isStoryMode || showCutscene)
		{
			switch (StringTools.replace(curSong," ", "-").toLowerCase())
			{
				...
				case 'killer-cough' | 'deathmatch-holo':
	...
	function startCountdown():Void
	{
		...
		if (SONG.song.toLowerCase() == 'monochrome' || SONG.song.toLowerCase() == 'killer-cough')
		{
			alpha = 0;
		...
		startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
		{
			...
			if (SONG.song.toLowerCase() != 'monochrome' && SONG.song.toLowerCase() != 'killer-cough' && SONG.song.toLowerCase() != 'endless' && SONG.song.toLowerCase() != 'triple-trouble')
			{
				switch (swagCounter)
				{
					case 0:
						...
						switch (curStage) {
							case 'mii-channel':
								var stageCurtains:FlxSprite = new FlxSprite(-449, -299);//.loadGraphic(Paths.image('eteled/stagecurtains'));
								stageCurtains.frames = Paths.getSparrowAtlas('eteled/miibuttons');
			...
			{	
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (!(curBeat >= 532 && curBeat < 536  && curSong.toLowerCase() == "expurgation" && dad.animOffsets.exists('Hank')) && !(curBeat == 516 && curSong.toLowerCase() == "split-ex") 
						&& !(curBeat >= 831 && curBeat < 848 && dad.curCharacter == 'bipolarmouse' && SONG.song.toLowerCase() == 'killer-cough') 
						&& !(curBeat >= 255 && curBeat < 256 && dad.curCharacter == 'bf-nene-scream' && SONG.song.toLowerCase() == 'killer-cough')
						&& !(curBeat >= 316 && curBeat < 320 && dad.curCharacter == 'blantad-scream' && SONG.song.toLowerCase() == 'killer-cough') && !(daNote.noteType == 2 && SONG.song.toLowerCase() == 'epiphany')
					...
					if (curBeat >= 831 && curBeat < 848 && dad.curCharacter == 'bipolarmouse' && SONG.song.toLowerCase() == 'killer-cough')
						dad.playAnim('singDOWN-alt', true);

					if (curBeat >= 316 && curBeat < 320 && dad.curCharacter == 'blantad-scream' && SONG.song.toLowerCase() == 'killer-cough')
	...
	private function popUpScore(daNote:Note):Void
		{
			...
			switch (curStage)
			{
				...
				case 'clubroomevil' | 'dokiclubroom-sayori' | 'dokiclubroom-natsuki' | 'dokiclubroom-yuri' | 'dokiclubroom-monika' | 'dokiclubroom2' | 'dokiclubroom3' | 'dokiclubroom-empty' | 'dokizoom':
				...
				if (!note.wasGoodHit)
				{
					...
					if (SONG.notes[Math.floor(curStep / 16)] != null)
					{
						...
						switch (curSong)
						{
							...
							case 'Tug-O-War':
								if (boyfriend.curCharacter == 'monisen' && boyfriend.altAnim == '-alt')
								{
							        health -= 0.02;
								}
							case 'Ugh':
